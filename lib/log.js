	...
	discord: (channelOrGuild, suppliedSettings) => {
		if (channelOrGuild instanceof discord.Guild) {
			...
			return channelOrGuild.systemChannel.send(module.exports.createEmbed(suppliedSettings))
					...
					.catch((error) => {
						if (error.message.equals('Missing Permissions')) { return false; } // Can't log due to missing permissions.
						module.exports.console(`Error logging to guild ${channelOrGuild}.`, error);
			...
			return channelOrGuild.send(module.exports.createEmbed(suppliedSettings))
					...
					.catch((error) => {
						if (error.message.equals('Missing Permissions')) { return false; } // Can't log due to missing permissions.
						module.exports.console(`Error logging to channel ${channelOrGuild}.`, error);
						return false; // Failure.
					});
